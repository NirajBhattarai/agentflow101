...
class PoolCalculatorAgent:
    ...
    def _parse_query_and_extract_data(self, query: str) -> dict:
        ...
        amount_match = re.search(r'swapping\s+([\d,]+\.?\d*)\s+(\w+)\s+to\s+(\w+)', query, re.IGNORECASE)
        if amount_match:
    ...
    def _calculate_optimal_allocations(
        ...
        if num_chains > 0:
            allocations_equal = {
                chain: total_amount / num_chains
                for chain in available_chains.keys()
            }
        ...
        if available_chains:
            ...
            if total_weight > 0:
                ...
                result_reserve = analyze_price_impact_for_allocation(
                    total_amount, allocations_reserve, pools_by_chain, token_in, token_out
                )
                scenarios.append({
        ...
        if "ethereum" in available_chains and len(available_chains) > 1:
            ...
            result_eth = analyze_price_impact_for_allocation(
                total_amount, allocations_eth_favored, pools_by_chain, token_in, token_out
            )
            scenarios.append({
        ...
        best_scenario = max(
            scenarios,
            key=lambda s: s["total_output"] / max(s["avg_price_impact"], 0.1)
        )
    ...
    async def invoke(self, query: str, session_id: str) -> str:
        ...
        try:
            ...
            if not parsed["total_amount"] or not parsed["token_in"] or not parsed["token_out"]:
                return json.dumps({
                    "recommended_allocations": {},
                    "total_output": 0.0,
                    "average_price_impact": 0.0,
            ...
            print(f"✅ Calculated optimal allocations: {result.get('recommended_allocations', {})}")
