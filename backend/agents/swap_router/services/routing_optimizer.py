...
def calculate_marginal_cost(
    ...
    if current_amount == 0:
        ...
        try:
            ...
            return impact.price_impact_percent + (
                gas_cost_usd / DEFAULT_ALLOCATION_STEP * 100
            )
        except Exception:
            return float("inf")
    ...
    try:
        ...
        marginal_impact = (
            impact_after.price_impact_percent - impact_before.price_impact_percent
        )
...
def optimize_routing(
    ...
    efficiency = (
        (net_output / (total_input_used * _get_spot_price(token_in, token_out))) * 100
        if total_input_used > 0
        else 0
    )

    # Build recommendation text
    recommendation_text = _build_recommendation_text(
        routes, total_input_used, total_output, avg_price_impact, total_gas_cost
    )
