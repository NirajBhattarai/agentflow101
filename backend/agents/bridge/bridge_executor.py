...
class BridgeTransaction(BaseModel):
    ...
    transaction_hash: Optional[str] = Field(default=None, description="Transaction hash if bridge is initiated")
...
class BridgeOption(BaseModel):
    ...
    is_recommended: Optional[bool] = Field(default=False, description="Is this the recommended option (lowest fee)")
...
class StructuredBridge(BaseModel):
    ...
    balance_check: Optional[BridgeBalanceCheck] = Field(default=None, description="Balance check result")
    bridge_options: Optional[list[BridgeOption]] = Field(default=None, description="Available bridge options with fees")
    transaction: Optional[BridgeTransaction] = Field(default=None, description="Bridge transaction details (if initiated)")
    requires_confirmation: Optional[bool] = Field(default=False, description="Whether explicit confirmation is required due to high amount")
    confirmation_threshold: Optional[float] = Field(default=None, description="The threshold amount that requires confirmation")
    amount_exceeds_threshold: Optional[bool] = Field(default=False, description="Whether the amount exceeds the confirmation threshold")
...
Return a structured JSON response with bridge transaction details.
    ...
    async def invoke(self, query: str, session_id: str) -> str:
        ...
        if "account:" in query_lower or "address:" in query_lower:
            account_match = re.search(r"(?:account|address)[:\s=]+(0\.0\.\d+|0x[a-fA-F0-9]{40})", query)
            if account_match:
                account_address = account_match.group(1)
        
        print(f"📊 Parsed: source={source_chain}, destination={destination_chain}, token={token_symbol}, amount={amount}, account={account_address}")
        ...
        token_address = token_addresses.get(source_chain, {}).get(token_symbol, "0x0000000000000000000000000000000000000000")
        ...
        confirmation_phrases = [
            "okay bridge", "ok bridge", "confirm bridge", "bridge now", 
            "proceed", "yes bridge", "go ahead", "execute bridge"
        ]
        ...
        if "initiate" in query_lower or "execute" in query_lower or has_confirmation:
            ...
            if requires_confirmation and not has_confirmation:
                # Don't initiate - return options with confirmation_required flag
                print(f"⚠️  Amount {amount_float} exceeds threshold {CONFIRMATION_THRESHOLD}. Requiring confirmation.")
                hardcoded_bridge["requires_confirmation"] = True
                hardcoded_bridge["confirmation_threshold"] = CONFIRMATION_THRESHOLD
                hardcoded_bridge["amount_exceeds_threshold"] = True
            else:
        ...
        if initiate_bridge and selected_protocol and (not requires_confirmation or has_confirmation):
            import random
            tx_hash = f"0x{''.join([random.choice('0123456789abcdef') for _ in range(64)])}"
            
            # Find the selected protocol's fee
            selected_option = next((opt for opt in bridge_options if opt["bridge_protocol"] == selected_protocol), bridge_options[0])
            ...
            print(f"🌉 Initiating bridge with {selected_protocol}, fee: {selected_option['bridge_fee']}")
