...
export const SwapRequirementsForm: React.FC<SwapRequirementsFormProps> = ({ args, respond }) => {
  ...
  const [chain, setChain] = useState("hedera");
  const [tokenInSymbol, setTokenInSymbol] = useState("HBAR");
  ...
  const filteredTokensIn = useMemo(() => {
    if (!tokenInSearch.trim()) return SWAPPABLE_TOKENS;
    const searchLower = tokenInSearch.toLowerCase();
    return SWAPPABLE_TOKENS.filter(
      (token) =>
  ...
  const filteredTokensOut = useMemo(() => {
    if (!tokenOutSearch.trim()) return SWAPPABLE_TOKENS;
    const searchLower = tokenOutSearch.toLowerCase();
    return SWAPPABLE_TOKENS.filter(
      (token) =>
        token.symbol.toLowerCase().includes(searchLower) ||
        token.name.toLowerCase().includes(searchLower),
  ...
  const handleTokenInSelect = (token: (typeof SWAPPABLE_TOKENS)[0]) => {
    setTokenInSymbol(token.symbol);
    setShowTokenInDropdown(false);
    setTokenInSearch("");
  };

  const handleTokenOutSelect = (token: (typeof SWAPPABLE_TOKENS)[0]) => {
  ...
  const validateForm = () => {
    ...
    if (tokenInSymbol === tokenOutSymbol) {
      newErrors.tokenOutSymbol = "Token out must be different from token in";
    }

    if (!amountIn.trim()) {
      newErrors.amountIn = "Please enter an amount to swap";
    } else {
      const amountNum = parseFloat(amountIn);
      if (isNaN(amountNum) || amountNum <= 0) {
        newErrors.amountIn = "Amount must be a positive number";
      }
    }

    if (!slippageTolerance.trim()) {
      newErrors.slippageTolerance = "Please enter slippage tolerance";
    } else {
      const slippageNum = parseFloat(slippageTolerance);
      if (isNaN(slippageNum) || slippageNum < 0 || slippageNum > 50) {
        newErrors.slippageTolerance = "Slippage tolerance must be between 0 and 50";
      }
    }
