/**
 * Token approval utilities for EVM-compatible chains
 *
 * Standard ERC20 token approval logic for EVM chains
 */
...
export async function approveToken(
  ...
  // EVM chains: estimate gas
  const overrides: ethers.Overrides = {};
  try {
    const estimatedGas = await tokenContract.approve.estimateGas(spender, approvalAmount);
    overrides.gasLimit = estimatedGas;
  } catch (error: any) {
    console.warn(`   Gas estimation failed, using default: ${error.message}`);
    overrides.gasLimit = 100_000; // Default for ERC20 approve
  }
...
/**
 * Check and approve token if needed for EVM chains
 */
export async function ensureTokenApproval(
  ...
  try {
    ...
    // Re-throw for EVM chains as they definitely need approval
    throw error;
  }
}
